...
chrome.cookies.getAll(
    ...
    async function (cookiesEcrm) {
        await cookiesEcrm.forEach((e) => {
            if (e.name == "browser-id") {
                domainNobi = [...domainNobi, e.domain];
            }
        });
        chrome.cookies.getAll(
            {
            ...
            async function (cookiesNobi) {
                await cookiesNobi.forEach((n) => {
                    if (n.name == "browser-id") {
                        domainNobi = [...domainNobi, n.domain];
                    }
                })
                console.log(domainNobi);
            }
        );
    }
);
                            ...
                            reader.onload = function () {
                                ...
                                if (check != null) {
                                    ...
                                    const username = checkUsername
                                        ...
                                        .toString();
                                    const regexPhone = new RegExp(`(?<!\\d[1-9]{1})((?:0|o)[\\.\\-\\s]?[1-9](?:[\\.\\-\\s]?[0-9o]){8,9}|(?:(?:\\+|00)\\d{1,4})[\\.\\-\\s]?[1-9](?:[\\.\\-\\s]?[0-9o]){6,9})(?!\\d)`, "gi");
                                    ...
                                    if (checkPhone || checkAddress) {
                                        ...
                                        fetch(`https://seller-vn.tiktok.com/api/v2/trade/orders/list?PIGEON_BIZ_TYPE=1&oec_region=VN&aid=4068&oec_seller_id=${shopIdApp}&language=vi-VN&locale=vi-VN`, requestOrder)
                                            ...
                                            .then(resOrder => {
                                                ...
                                                var myHeadersNobi = new Headers();
                                                myHeadersNobi.append("Content-Type", "application/json");

                                                var rawNobi = JSON.stringify({
                                                    "Username": `${username}`,
                                                    "Message": `${mess}`,
                                                    "OrderId": `${orderId}`,
                                                    "ShopId": `${shopIdApp}`
                                                ...
                                                console.log(rawNobi);
                                                var requestUpdateCus = {
                                                    method: 'POST',
                                                    headers: myHeadersNobi,
                                                    body: rawNobi,
                                                    redirect: 'follow'
                                                };
                                                domainNobi.forEach((dm) => {
                                                    fetch(`https://${dm}/api/tiktok/extension`, requestUpdateCus)
                                                    .then((resNobi) => resNobi.json())
                                                    .then(resNobi => console.log(resNobi))
                                                    .catch(error => console.log('error', error));
                                                })
                                                // fetch(`https://basev2-ftr-extension-tiktok.ecrm.vn/api/tiktok/extension`, requestUpdateCus)
                                                //     .then((resNobi) => resNobi.json())
                                                //     .then(resNobi => console.log(resNobi))
                                                //     .catch(error => console.log('error', error));
                                            })
